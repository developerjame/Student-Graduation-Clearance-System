<li class="nav-header">
                </li>
                <li class="active">
                <a href="student.php"><i class="fa fa-th-large"></i> <span class="nav-label">Dashboard</span></a></li>
				<li>
                    <a href="#"><i class="fa fa-user"></i> <span class="nav-label">Account</span><span class="fa arrow"></span></a>
                    <ul class="nav nav-second-level collapse">
                 <li><a href="edit-photo.php">Edit Photo</a></li>
				  <li><a href="changepassword.php">Change Password</a></li>

                    </ul>
                </li>
				
                <li>
                    <a href="#"><i class="fa fa-dollar"></i> <span class="nav-label">fee</span><span class="fa arrow"></span></a>
                    <ul class="nav nav-second-level collapse">
                 <li><a href="pay-fee.php">Pay fee</a></li>
				  <li><a href="fee-history.php">payment History</a></li>
                    </ul>
                </li>
                <li><a href="" onclick="myfunction()">Initiate Clearance</a></li>
                <script>
                    function myfunction(){
                        alert("You have successfully initiated")
                    }
                </script>

			    